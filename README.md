...
六角星 121 格中國跳棋，標準跳法。首頁分成兩條路，兩者的設定畫面各自獨立：

- **線上對戰**：大廳看得到目前有哪些房間，可以**加入對戰**或**加入觀戰**；
  座位補滿後由**房主按「開始遊戲」**才開局，人數不足可用電腦 AI 補位（需要跑 Node 伺服器）
...
1. 首頁輸入暱稱 → 進「線上對戰」
2. **大廳**列出目前所有公開房間，每一列標示「等待中／進行中」、已入座人數與觀戰人數
   - **加入對戰**：房間已滿或已經開打時會自動變灰
   - **加入觀戰**：任何狀態都能按，開打中的房間也能中途進去看
3. 自己開房的話，房間頁會給你**房號**與**邀請連結**（`https://.../?room=ABCD`）
4. 朋友點邀請連結進來，會看到房間現況與兩顆按鈕，自己選要下棋還是觀戰
5. 座位補滿（真人或 AI）後，**房主**按「開始遊戲」才正式開局

觀戰者沒有座位、不能走子，但看得到完整棋盤、棋譜，也可以在聊天室發言（會標示「觀眾」）。

### 房間生命週期

房裡沒有真人玩家的房間會被關閉（只剩 AI 或只剩觀戰者都算沒人），觀戰者會被請回大廳。
伺服器把「主動離開」和「連線中斷」當成兩件事：

| 情況 | 前端行為 | 伺服器處理 |
| --- | --- | --- |
| 按下「離開」 | 先送 `leave` 再關連線 | 立刻釋出座位；若房裡沒真人了就**馬上關房** |
| 重新整理 / 網路瞬斷 / 關分頁 | 只有連線斷掉 | 保留座位與 token，等 `ROOM_GRACE_MS`（預設 **45 秒**） |

所以**對局中重新整理不會失去房間** —— 重連後前端會拿 localStorage 裡的 token 送 `rejoin` 回到原座位，
棋盤、棋譜、聊天記錄都還在。緩衝期內該房間不會出現在大廳列表，座位會標示「斷線中」。
緩衝期過了還沒回來，座位就釋出；房裡一個真人都沒有時，房間關閉。

```bash
ROOM_GRACE_MS=90000 npm start   # 網路不穩的場合可以拉長
ROOM_GRACE_MS=0 npm start       # 不留緩衝，斷線即關房
```

### 平板與觸控

- 走法提示點有放大的隱形點擊區（半徑等於格距的一半），手指點得到又不會誤觸隔壁格
- 觸控裝置上提示點不依賴 hover，一律畫得夠明顯；按鈕最小高度 46px
- 輸入框在觸控裝置上用 16px，避免 iOS 聚焦時自動放大頁面
- 平板直向時棋盤佔滿上半部、資訊面板橫排在下方；橫向則收窄側欄讓棋盤變大

## 快速開始

需要 **Node.js 24 LTS** 以上（`.nvmrc` 已指定 24）。
